...
dataset_out = nc.Dataset(direct + 'maqs-fidas-1_' + str(date_file_label) + '_surface-met_' + str(version_number) + '.nc', 'w', format='NETCDF4_CLASSIC')
...
latitudes[:] = 53.456636
longitudes[:] = -2.214244
Flag_Met_Byte = np.array(fidas_Met['Met_Flag']).astype(np.ubyte)
AirPres[:] = fidas_Met['Pressure (mbar)'].values
qc_flag_AirPres[:] = Flag_Met_Byte
AirTemp[:] = fidas_Met['Temperature (K)'].values
qc_flag_AirTemp[:] = Flag_Met_Byte
RelHum[:] = fidas_Met['Humidity (%)'].values
qc_flag_RelHum[:] = Flag_Met_Byte
