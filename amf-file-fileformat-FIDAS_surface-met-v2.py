...
version_number = 'v0.5'
direct = 'D:/'#'C:/Users/mbexknm5/Dropbox/Python_Scripts/'

dataset_out = nc.Dataset(direct + 'maqs-fidas-1_maqs_20210719_surface-met_v0.5.nc', 'w', format='NETCDF4_CLASSIC')
...
dataset_out.history = ' Acquired 2019, Data processed Aug2021'
...
times.axis = 'T'
times.valid_min = fidas_Data['TimeSecondsSince'][0]#(timeline[0]-datetime.datetime(1970,1,1,0,0,0)).total_seconds()
times.valid_max = fidas_Data['TimeSecondsSince'][-1]#(timeline[-1]-datetime.datetime(1970,1,1,0,0,0)).total_seconds()
...
day_of_year.valid_min = timeline[0].timetuple().tm_yday
day_of_year.valid_max = timeline[-1].timetuple().tm_yday
...
year.standard_name = ''
year.long_name = 'Year'
year.valid_min = timeline[0].year
year.valid_max = timeline[-1].year
...
AirTemp.valid_min = fidas_Met['Temperature (deg C)'].min()
AirTemp.valid_max = fidas_Met['Temperature (deg C)'].max()
...
times[:] = fidas_Data['TimeSecondsSince'].values#nc.date2num(timeline[:],times.units)
day_of_year[:] = fidas_Data['day_year'].values
year[:] = fidas_Data['year'].values
month[:] = fidas_Data['month'].values
day[:] = fidas_Data['day'].values
hour[:] = fidas_Data['hour'].values
minute[:] = fidas_Data['minute'].values
second[:] = fidas_Data['second'].values
latitudes[:] = 53.456636
longitudes[:] = -2.214244
AirPres[:] = fidas_Met['Pressure (mbar)'].values
qc_flag_AirPres[:] = fidas_Met['Met_Flag'].values
AirTemp[:] = fidas_Met['Temperature (deg C)'].values
qc_flag_AirTemp[:] = fidas_Met['Met_Flag'].values
RelHum[:] = fidas_Met['Humidity (%)'].values
qc_flag_RelHum[:] = fidas_Met['Met_Flag'].values
