...
dataset_out = nc.Dataset(direct + 'maqs-fidas-1_maqs_20210719_PM-concentration_v0.5.nc', 'w', format='NETCDF4_CLASSIC')
...
dataset_out.history = ' Data processed 3Aug2021'
...
qc_flag_PM1[:] = fidas_PM['PM_Flag'].values
PM2p5[:] = fidas_PM['PM2.5 (ug/m3)'].values
qc_flag_PM2p5[:] = fidas_PM['PM_Flag'].values
PM10[:] = fidas_PM['PM10 (ug/m3)'].values
qc_flag_PM10[:] = fidas_PM['PM_Flag'].values
PM_Cn[:] = fidas_PM['Cn (P/cm3)'].values
qc_flag_PM_Cn[:] = fidas_PM['PM_Flag'].values
